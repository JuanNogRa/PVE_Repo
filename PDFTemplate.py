...
class Report_eficiencia:
        def foot2(self,pdf,doc):
                width,height = A4
                pdf.saveState()
                pdf.setTitle("REPORTE DE PRUEBA DE EFICIENCIA")
                pdf.setAuthor("Universidad del Valle")
                pdf.setFont('Helvetica-Bold',24)
                pdf.drawCentredString(width/2.0, height-63, "REPORTE DE PRUEBA DE EFICIENCIA")
                pdf.setFont('Times-Roman',9)
                #pdf.drawString(width-1*inch, 0.1 * inch,'Página '+"%d" % doc.page+' de '+"%d" % 3)

        def get_doc(self, buffer, encargado, fecha, Datos_vehiculo, data_graph, data_graph_perfil, data_graph_perfil_r, Carga, Consumo, Rango):
                width,height = A4
                ...
                contents.append(table)
                datos = (
                                ('Fecha y hora de elaboración de la prueba de consumo:', fecha[0]),
                                ('Fecha y hora de elaboración de la prueba de rango:', fecha[1]),
                                ('Fecha y hora de elaboración del reporte:', fecha[2]),
                                ('Fecha de calibración sensores:', fecha[3]),
                                )
                table = LongTable(data = datos,colWidths=((1.0+3.5)*inch,2.95*inch),
                                style = [
                                        ('GRID',(0,0),(2,3),0.5,colors.grey),
                                        ('GRID',(0,2),(2,3),0.5,colors.grey),
                                        ('FONTNAME', (0,0), (0,3), 'Helvetica-Bold'),
                                        ],
                                hAlign='LEFT'
                                ,splitByRow=True
                ...
                style_title = ParagraphStyle('Titulos',
                           ...
                           spaceAfter=14)
                datos = (
                                ('Energía cargada (kWh)' , str(Carga)+' kWh'),
                                ('Consumo energetico (kWh/km)' , str(Consumo)+' kWh/km'),
                                ('Rango (km)' , str(Rango)+' km'),                         
                        )
                table = LongTable(data = datos,colWidths=(4*inch,3.45*inch),
                                style = [
                                        ('GRID',(0,0),(1,3),0.5,colors.grey),
                ...
                contents.append(table)
                Title=Paragraph("Gráfica de potencia instantanea (kW) vs tiempo (h)", style_title)
                contents.append(Title)
                ...
                Title=Paragraph("", style_title)
                contents.append(Title)
                Title=Paragraph("Gráficas de perfil de velocidad (km/h) vs tiempo (s) prueba de consumo", style_title)
                contents.append(Title)
                ...
                lp.height = 125
                lp.width = 450
                lp.data = data_graph_perfil
                lp.joinedLines = 1
                lp.strokeColor = colors.black
                drawing.add(lp)
                contents.append(drawing)
                
                Title=Paragraph("Gráficas de perfil de velocidad (km/h) vs tiempo (s) prueba de rango", style_title)
                contents.append(Title)
